...
/**
 * Service to interact with Gemini API for HumedBot.
 */
...
export const getGeminiResponse = async (prompt: string) => {
  try {
    const response = await ai.models.generateContent({
      ...
      contents: prompt,
      config: {
        systemInstruction: `Eres HumedBot, un asistente experto en el Humedal de Techo en Bogotá, Colombia.
...
Siempre responde en ESPAÑOL y con un formato muy claro y fácil de leer:
- Empieza con un TÍTULO corto en mayúsculas (máx. 6 palabras).
...
- Termina con una línea que empiece por "Recuerda:" con una recomendación práctica para cuidar el humedal.
